...
#define FILEPATH "/tmp/imsi.conf"
#define FILEPATH2 "/tmp/person.dat"
...
/* 分割字符串 */
int spilt(char dst[][32], char *str, const char *spl)
{
	int n = 0;
	char *result = NULL;
...
void readfile(void)
{
	...
	read_item("iwmoncfg.bbb.device_id", value);
	oldtime = time(NULL);
	...
	if((fd = fopen(FILEPATH, "rb")) != NULL)
	{
		...
		cnt = spilt(dst, last_line, ",");
		if(cnt == 0)
			return;
		StrTrim(dst[1]);
		sprintf(imsiinfo, "%d,%s,%s,%s,%s\n", 1, value, dst[2], dst[0], dst[1]);
		//printf("%s\n", );
		if(strlen(imsiinfo) < 40)
			return;
		sendData(imsiinfo);
		close(cli_nodify_fd);
	}
